	...
	struct SW_RFB *prSwRfb)
{
	...
	prRadiotapInfo->ucFrag =
		HAL_MAC_CONNAC3X_RX_STATUS_IS_FRAG(prRxStatus);
	...
	if (prRadiotapInfo->ucTxMode >= TX_RATE_MODE_HE_SU) {
		...
		prRadiotapInfo->u2BeamChange =
		    HAL_MAC_CONNAC3X_RX_VT_GET_BEAM_CHANGE(prRxStatusGroup5);
		...
		prRadiotapInfo->u2SpatialReuse1 =
		    HAL_MAC_CONNAC3X_RX_VT_GET_SPATIAL_REUSE1(prRxStatusGroup5);
		prRadiotapInfo->u2SpatialReuse2 =
		    HAL_MAC_CONNAC3X_RX_VT_GET_SPATIAL_REUSE2(prRxStatusGroup5);
		prRadiotapInfo->u2SpatialReuse3 =
		    HAL_MAC_CONNAC3X_RX_VT_GET_SPATIAL_REUSE3(prRxStatusGroup5);
		prRadiotapInfo->u2SpatialReuse4 =
		    HAL_MAC_CONNAC3X_RX_VT_GET_SPATIAL_REUSE4(prRxStatusGroup5);
...
{
	struct tx_free_done_rpt *rpt =
		(struct tx_free_done_rpt *)prSwRfb->pucRecvBuff;
