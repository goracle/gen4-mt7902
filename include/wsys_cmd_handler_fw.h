...
enum ENUM_CMD_ID {
	...
	CMD_ID_ONE_TIME_CAL = 0xD6,		/* 0xD6 (Set) */
#endif
...
#endif
#if CFG_SUPPORT_ONE_TIME_CAL
	EVENT_ID_ONE_TIME_CAL = 0xD6,		/* 0xD6 (Set) */
#endif
#if CFG_SUPPORT_NAN
...
struct CMD_SCAN_REQ {
	...
	uint16_t         u2IELen;
	uint8_t          aucIE[1];  /*depends on u2IELen*/
};
...
#endif
#define SINGLE_SKU_LEGACY_PARAM_NUM 32

struct CMD_SKU_TABLE_TYPE {
	int8_t i1PwrLimit[SINGLE_SKU_PARAM_NUM];
};

struct CMD_SKU_LEGACY_TABLE_TYPE {
	int8_t i1LegacyPwrLimit[SINGLE_SKU_LEGACY_PARAM_NUM];
};
...
struct CMD_SET_TXPOWER_COUNTRY_TX_LEGACY_POWER_LIMIT_PER_RATE {
	...
	/* DWORD_11 ~ - Legacy Tx power limit values*/
	struct CMD_TXPOWER_CHANNEL_LEGACY_POWER_LIMIT_PER_RATE
	    rChannelLegacyPowerLimit[];
};
